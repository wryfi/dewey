...
from dewey.utils.dotutils import flatten_dict
from environments.models import Environment, Host, Role, Safe, SafeAccessControl, Secret, Vault
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--directory', '-d', help='directory from which to import or export', required=True)
        parser.add_argument('--environment', '-e', help='environment (prod, dev, etc) the secrets go with', required=True)
    ...
    def handle(self, *args, **options):
        ...
        if options['action'] == 'import':
            return self.import_secrets()
        if options['action'] == 'export':
            return self.export_secrets()
    ...
    def import_host_secrets(self):
        ...
        for secrets_file in os.listdir(host_secrets_dir):
            ...
            try:
                ...
                self.stdout.write('host {} does not exist'.format(hostname))
            ...
            if 'hosts' in secrets_yaml.keys():
                for host in secrets_yaml['hosts']:
                    if re.match(r'^[A-Za-z._-]+$', host):
                        ...
                        self.stdout.write('WARNING! unsupported host regex: {}'.format(host))
    ...
    def export_secrets(self):
        print(self.action, self.directory)
